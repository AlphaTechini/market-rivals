...
- [wallet-provider.ts](./wallet-provider.ts) discovers installed EIP-6963 wallets and remembers the user's selected provider without storing keys or addresses.
...
To find multi-wallet discovery and provider selection visit [wallet-provider.ts](./wallet-provider.ts).
